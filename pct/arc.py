...
# %% auto #0
__all__ = ['ARCFitnessFactory', 'BaseARCFitness', 'SumSquareOfDiff', 'AverageMaxOfDiff', 'Euclidean', 'ARCDataProcessor',
           'ARCEnv']

# %% ../nbs/15_arc.ipynb #ea9781fd
import os, math
import numpy as np
import gym
...
class BaseARCFitness(ABC):
    ...
    def get_dim_metric(self, output_array=None, env_array=None, control_set=None):   
        dim_metric = 0
...
class SumSquareOfDiff(BaseARCFitness):
    ...
    def __call__(self, output_array=None, env_array=None, control_set=None):
        ...
        dim_metric = self.get_dim_metric(output_array, env_array, control_set)
...
# %% ../nbs/15_arc.ipynb #a70dca12
class Euclidean(BaseARCFitness):
    "A function that sums the square of the diffrences of two arrays."
    ...
    def __call__(self, output_array=None, env_array=None, control_set=None):
        ...
        if 'cells' in control_set:
            ...
            diff = np.where(np.isnan(diff), 0, diff)
            diff = np.where(np.isnan(env_array[:output_array.shape[0], :output_array.shape[1]]), 10, diff)
            element_metric = np.sqrt(np.sum(diff ** 2))

            abs_diff = np.abs(diff)
            maximum = np.max(abs_diff)
            if maximum < 0.5:
                element_metric = maximum
    ...
    class Factory:
        def create(self): return Euclidean()     

# %% ../nbs/15_arc.ipynb #351e17a3
...
class ARCEnv(gym.Env):
    ...
    def initialise(self, properties, arc_dict):
        ...
        self.abs_tol_ARC_display = properties['tolerances'].get('atARCdisplay', 0.5)
